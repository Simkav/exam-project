...
export function* activeContestsSaga(action){
    ...
    try{
        const {data}=yield  contest.getActiveContests(action.data);
...
export  function* customerContestsSaga(action){
    yield put({type: ACTION.GET_CONTESTS_ACTION_REQUEST});
    try{
        const {data}=yield contest.getCustomersContests(action.data);
        yield  put({type: ACTION.GET_CONTESTS_ACTION_SUCCESS, data: data});
    }
    catch (e) {
        yield put({type: ACTION.GET_CONTESTS_ACTION_ERROR, error: e.response});
...
export  function* updateContestSaga(action) {
    ...
    try {
        const {data}= yield contest.updateContest(action.data);
...
export  function* dataForContestSaga(action){
    ...
    try{
        const {data} = yield contest.dataForContest(action.data);
...
export function* getContestByIdSaga(action){
    ...
    try{
        const {data}=yield  contest.getContestById(action.data);
        const {Offers}=data;
        delete data.Offers;
...
export  function* downloadContestFileSaga(action){
    ...
    try{
        const {data}=yield  contest.downloadContestFile(action.data);
